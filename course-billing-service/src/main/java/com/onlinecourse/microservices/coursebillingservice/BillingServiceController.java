...
public class BillingServiceController {
	...
	return new BillingCourseBean (courseId, response.getCourseName(), response.getCoursePrice(), configuration.getTax(),
		   configuration.getDiscount(), response.getCoursePrice()-((response.getCoursePrice()*configuration.getDiscount())/100)
		   +((response.getCoursePrice()*configuration.getTax())/100));
	}
